...
3. **Token Pair Generation**:
   ...
   - **Access Token**: A JWT containing the `user_id` (`uid`) and the `session_id` (`sid`).
4. **Response**: The client receives the JWT and a combined refresh string: `session_id.refresh_secret`.

### 2.2 Token Rotation (Refresh)
When the access token expires, the client calls `/api/refresh`:
1. **Verification**: The server parses the combined refresh string and lookups the session in the DB.
...
1. **Mark Revoked**: The `revoked_at` column for that specific `session_id` is set to the current timestamp.
2. **Immediate Effect**:
   - Any further attempts to use that Refresh Token will fail.
   - The Middleware will reject any Access Tokens linked to that `session_id`.
...
2. Extracting the `session_id` (`sid`) from the JWT claims.
3. **Session Verification**: Querying the `sessions` table to ensure the specific session has not been revoked. This allows for immediate global logout across all devices if a session is compromised.
